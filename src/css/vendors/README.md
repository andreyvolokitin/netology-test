...
Note: prefer using npm-installed packages instead of putting files into `vendors/`. 
To import packages (or files inside `vendors/` folder) inside `vendor.scss` file use SASS imports with `.css` extension in order to not embed them directly into the file but to convert them to regular CSS @imports. Then webpack will be able to bundle them as usual. 
